...
describe('AlbumList', () => {
  ...
  const markupNothingFound = `No album was found`;
  ...
  it('should create and append the markupwhen more than one data', () => {
    const element = document.createElement('div');
  ...
  it('should display `No album found` when data is empty ([])', () => {
    const element = document.createElement('div');
    renderAlbums([], element);

    expect(element.innerHTML).to.be.eql(markupNothingFound);
  });
